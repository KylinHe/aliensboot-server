...
func (this *AgarGameFactory) NewGame(handler game.Handler) game.Game {
	return &AgarGame{CommonGame: &game.CommonGame{Handler: handler}}
}

type AgarGame struct {
	*game.CommonGame
...
//接收玩家数据，同步给其他玩家
func (game *AgarGame) AcceptPlayerData(playerID int64, data string) {
	//游戏结束不处理游戏内的数据转发
...
//func (game *AgarGame) AcceptPlayerMessage(playerID int64, request interface{}, response interface{}) {
